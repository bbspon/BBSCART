...
const Orders = () => {
  ...
  // ✅ Safe filter to avoid null user_id or name
  const filteredOrders = (orders || [])
    .filter((order) => order && order.user_id && order.user_id.name)
    .filter((order) =>
          ...
          <main>
            ...
            <div className="order bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-6 mt-6">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
                <h3 className="text-xl font-bold text-blue-700 dark:text-blue-300 tracking-tight">
                  Recent Orders
                </h3>
              ...
              <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
                <table className="w-full table-auto border-collapse text-sm">
                  <thead className="bg-blue-50 dark:bg-gray-800">
                    <tr>
                      <th className="font-Poppins p-3 pl-6 text-left font-semibold text-blue-700 dark:text-blue-200 tracking-wide">
                        User
                      </th>
                      <th className="font-Poppins p-3 text-left font-semibold text-blue-700 dark:text-blue-200 tracking-wide">
                        Date Order
                      </th>
                      <th className="font-Poppins p-3 text-left font-semibold text-blue-700 dark:text-blue-200 tracking-wide">
                        Status
                      ...
                      >
                        <td className="p-3 pl-6 align-middle font-medium text-gray-900 dark:text-gray-100 flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full flex items-center justify-center bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200 font-bold text-lg uppercase border border-gray-200 dark:border-gray-700">
                            {order?.user_id?.name
                              ? order.user_id.name.charAt(0)
                              : "?"}
                          </div>
                          <span>{order?.user_id?.name || "Unknown User"}</span>
                        </td>
