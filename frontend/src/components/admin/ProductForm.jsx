...
const ProductForm = ({
  ...
  const [productImage2Preview, setProductImage2Preview] = useState(null);
  ...
  const [galleryImgUrls, setGalleryImgUrls] = useState(""); // pipe or comma separated
  ...
  const validateVendor = () => {
    ...
    if (!productData.is_variant === true) {
      ...
      if (!productData.product_img)
        formErrors.product_img = "Product image is required";
    }
    ...
    if (!productData.category_id)
      formErrors.category_id = "Product Category is required";
    if (!productData.subcategory_id)
      formErrors.subcategory_id = "Product Subcategory is required";

    if (productData.is_variant === true) {
      if (!variantData.variant_name && !productData.variants.length < 1)
        formErrors.variant_name = "Product Variant name is required";
      if (!variantData.price && !productData.variants.length < 1)
        formErrors.vprice = "Product Variant price is required";
      if (!variantData.stock && !productData.variants.length < 1)
        formErrors.vstock = "Product Variant stock is required";
      if (!variantData.SKU && !productData.variants.length < 1)
        formErrors.VSKU = "Product Variant SKU is required";
      if (!variantData.variant_img && !productData.variants.length < 1)
        formErrors.variant_img = "Product Variant image is required";
    }
    return formErrors;
  };
  ...
  const addVariant = () => {
    setProductData((prev) => {
      ...
      const newVariant = {
        ...
        variant_gallery_imgs: variantData.variant_gallery_imgs
          ? [...variantData.variant_gallery_imgs]
          : [], // Preserve files
      };
  ...
  const handleAttributeChange = (attrIndex, field, value) => {
    setVariantData((prev) => {
      ...
      const updatedAttributes = Array.isArray(prev.attributes)
        ? [...prev.attributes]
        : [];
  ...
  const handleProductImageChange = (e, type) => {
    ...
    if (type === "product") {
      setProductData((prev) => ({ ...prev, product_img: file }));
      setProductImagePreview(URL.createObjectURL(file));
    } else if (type === "product2") {
      setProductData((prev) => ({ ...prev, product_img2: file }));
      setProductImage2Preview(URL.createObjectURL(file));
    } else {
      // variant
      setVariantData((prev) => ({ ...prev, variant_img: file }));
    ...
    setVariantGalleryPreviews((prevPreviews) => {
      ...
      setVariantData((prevData) => {
        const updatedExisting = [
          ...(prevData.existing_variant_gallery_imgs || []),
        ];
  ...
  const handleSubmit = (e) => {
    ...
    submissionData.append(
      "is_variant",
      productData.is_variant ? "true" : "false"
    );
    ...
    if (productData.is_variant) {
      ...
      if (productData.product_img2 instanceof File) {
        submissionData.append("product_img2", productData.product_img2);
      }
    ...
    console.log("Saved product:", {
      ...
      product_img2: productData.product_img2,
      gallery: productData.gallery_imgs,
    });
  };
  ...
  useEffect(() => {
    if (product) {
      ...
      if (Array.isArray(product.gallery_imgs)) {
        const urls = product.gallery_imgs.map(
  ...
  useEffect(() => {
    ...
    const existingPreviews = (
      variantData.existing_variant_gallery_imgs || []
    ).map((url) => ({
      preview:
        typeof url === "string"
    ...
    const newPreviews = (variantData.variant_gallery_imgs || []).map(
      (file) => ({
        preview:
          typeof file === "string"
    ...
    if (variantData.variant_img) {
      if (typeof variantData.variant_img === "string") {
        setVariantImagePreview(
          import.meta.env.VITE_API_URL + variantData.variant_img
        );
    ...
    <div className="formSec bg-white shadow-md rounded-2xl h-[85%] relative flex items-center justify-center z-50 fixed inset-0 bg-black">
      <div className="input-box-form p-3 overflow-y-auto h-full w-full rounded-2xl max-w-2xl mx-auto shadow-2xl border border-gray-200 relative flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 rounded-t-2xl bg-gradient-to-r from-blue-50 to-white sticky top-0 z-10">
          ...
          <button
            className="text-2xl text-gray-400 hover:text-red-500 transition popup-close"
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                ...
                <div className="input-item">
                  ...
                  <input
                    ...
                    className={`w-full p-2 border rounded-lg ${
                      errors.name ? "border-red-700" : "mb-4 "
                    }`}
                  />
                  {errors.name && (
              ...
              <div className="col-span-1 md:col-span-2">
                <div className="input-item">
                  ...
                  <textarea
                    ...
                    className={`w-full p-2 border rounded-lg ${
                      errors.description ? "border-red-700" : "mb-4"
                    }`}
                  />
                  {errors.description && (
                    <div className="text-red-800">{errors.description}</div>
                  )}
                </div>
              </div>
              {/* Price */}
              <div>
                <div className="input-item">
                  ...
                  <input
                    ...
                    placeholder="Price"
                    className={`w-full p-2 border rounded-lg ${
                      errors.price ? "border-red-700" : "mb-4"
                    }`}
                  />
                  {errors.price && (
                    <div className="text-red-800">{errors.price}</div>
                  )}
                </div>
              </div>
              {/* Stock */}
              <div>
                <div className="input-item">
                  ...
                  <input
                    ...
                    placeholder="Stock"
                    className={`w-full p-2 border rounded-lg ${
                      errors.stock ? "border-red-700" : "mb-4"
                    }`}
                  />
                  {errors.stock && (
                    <div className="text-red-800">{errors.stock}</div>
                  )}
                </div>
              </div>
              {/* SKU */}
              <div>
                <div className="input-item">
                  ...
                  <input
                    ...
                    placeholder="SKU"
                    className={`w-full p-2 border rounded-lg ${
                      errors.SKU ? "border-red-700" : "mb-4"
                    }`}
                  />
                  {errors.SKU && (
                    <div className="text-red-800">{errors.SKU}</div>
                  )}
                </div>
              </div>
              {/* Brand */}
              <div>
                <div className="input-item">
                  ...
                  <input
                    ...
                    placeholder="Brand"
                    className={`w-full p-2 border rounded-lg ${
                      errors.brand ? "border-red-700" : "mb-4"
                    }`}
                  />
                  {errors.brand && (
                    <div className="text-red-800 ">{errors.brand}</div>
                  )}
                </div>
              </div>
              {/* Weight */}
              <div>
                <div className="input-item">
                  ...
                  <input
                    ...
                    placeholder="Weight"
                    className={`w-full p-2 border rounded-lg ${
                      errors.weight ? "border-red-700" : "mb-4"
                    }`}
                  />
                  {errors.weight && (
                    <div className="text-red-800">{errors.weight}</div>
                  )}
                </div>
              </div>
              {/* Dimensions (row of 3) */}
                ...
                <div className="input-item">
                  ...
                  <Select
                    ...
                    className={`w-full border rounded-lg ${
                      errors.category_id ? "border-red-700" : ""
                    }`}
                    name="category_id"
                  />
                  {errors.category_id && (
                    <div className="text-red-800">{errors.category_id}</div>
                  )}
                </div>
              </div>
              {/* Subcategory */}
                  ...
                  <Select
                    ...
                    value={
                      subcategoriesOptions.find(
                        (option) => option.value === productData.subcategory_id
                      ) || null
                    }
                    onChange={handleSelectChange}
                    ...
                    isSearchable
                    className={`w-full border rounded-lg ${
                      errors.subcategory_id ? "border-red-700" : ""
                    }`}
                    name="subcategory_id"
                  />
                  {errors.subcategory_id && (
                    <div className="text-red-800">{errors.subcategory_id}</div>
                  )}
                </div>
              </div>
              {/* Seller (Vendor) */}
              <div>
                <div className="input-item">
                  ...
                  <Select
                    ...
                    options={vendors}
                    value={
                      vendors.find((o) => o.value === productData.seller_id) ||
                      null
                    }
                    onChange={handleSelectChange}
                    isSearchable
                  ...
                  {errors.seller_id && (
                    <div className="text-red-800">{errors.seller_id}</div>
                  )}
                </div>
              </div>
              ...
              <div>
                <div className="input-item">
                  <label className="block text-[14px] font-medium text-secondary">
                  ...
                  <input
                    ...
                    className={`w-full p-2 border rounded-lg ${
                      errors.product_img ? "border-red-700" : "mb-4"
                    }`}
                  />
                  {errors.product_img && (
                    <div className="text-red-800">{errors.product_img}</div>
                  )}
                </div>
              </div>
              {/* Secondary Product Image Upload */}
              <div>
                <div className="input-item">
                  <label className="block text-[14px] font-medium text-secondary">
                  ...
                  {productImage2Preview && (
                    <div className="mb-2 w-16 h-16">
                      <img
                        src={productImage2Preview ?? ""}
                  ...
                  <input
                    ...
                    name="product_img2"
                    accept="image/*"
                    onChange={(e) => handleProductImageChange(e, "product2")}
                    className="w-full p-2 border rounded-lg"
                  />
                </div>
              </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      {galleryImagePreviews.map((src, index) => (
                        <div key={index} className="relative w-16 h-16 mb-2">
                          ...
                          <button
                            className="text-2xl bg-white w-6 h-6 rounded-full flex items-center text-red-700 hover:text-red-500 transition absolute top-1 right-1"
                            onClick={() => handleRemoveGalleryImage(index)}
                            type="button"
                          >
                            <i className="ri-close-circle-line"></i>
            ...
            <div className="w-full">
              <div className="input-item flex flex-wrap gap-3 items-center">
                <input
                  ...
                  id="is_variant"
                  name="is_variant"
                  checked={productData.is_variant}
                  onChange={handleChange}
                />
                <label
                  htmlFor="is_variant"
                  className="block text-[14px] font-medium text-secondary"
                >
                  {" "}
              ...
              <div className="variant-manager-sec mt-5 w-full">
                ...
                <div className="w-full">
                  <input
                    ...
                    className={`w-full p-2 border rounded-lg ${
                      errors.variant_name ? "border-red-700" : "mb-4"
                    }`}
                    value={variantData.variant_name}
                    onChange={handleVariantChange}
                  />
                  {errors.variant_name && (
                    <div className="text-red-800">{errors.variant_name}</div>
                  )}
                </div>
                ...
                <div className="w-full mt-2">
                  <input
                    ...
                    className={`w-full p-2 border rounded-lg ${
                      errors.vprice ? "border-red-700" : "mb-4"
                    }`}
                    value={variantData.price}
                    onChange={handleVariantChange}
                  />
                  {errors.vprice && (
                    <div className="text-red-800">{errors.vprice}</div>
                  )}
                </div>
                ...
                <div className="w-full mt-2">
                  <input
                    ...
                    placeholder="Stock"
                    className={`w-full p-2 border rounded-lg ${
                      errors.vstock ? "border-red-700" : "mb-4"
                    }`}
                    value={variantData.stock}
                    onChange={handleVariantChange}
                  />
                  {errors.vstock && (
                    <div className="text-red-800">{errors.vstock}</div>
                  )}
                </div>
                ...
                <div className="w-full mt-2">
                  <input
                    ...
                    placeholder="SKU"
                    className={`w-full p-2 border rounded-lg ${
                      errors.VSKU ? "border-red-700" : "mb-4"
                    }`}
                    value={variantData.SKU}
                    onChange={handleVariantChange}
                  />
                  {errors.VSKU && (
                    <div className="text-red-800">{errors.VSKU}</div>
                  )}
                </div>
                ...
                <div className="w-full mt-3">
                  <div className="input-item">
                    ...
                    <input
                      ...
                      className={`w-full p-2 border rounded-lg ${
                        errors.variant_img ? "border-red-700" : "mb-4"
                      }`}
                    />
                    {errors.variant_img && (
                      <div className="text-red-800">{errors.variant_img}</div>
                    )}
                    {variantImagePreview && (
                      <div className="mt-2">
                        <img
                    ...
                    variantData.attributes.length > 0 ? (
                      ...
                      <div className="flex space-x-2 mb-2">
                        <input
                          ...
                          value=""
                          onChange={(e) =>
                            handleAttributeChange(0, "key", e.target.value)
                          }
                          className="w-1/2 p-2 border border-gray-300 rounded"
                        />
                        <input
                          ...
                          value=""
                          onChange={(e) =>
                            handleAttributeChange(0, "value", e.target.value)
                          }
                          className="w-1/2 p-2 border border-gray-300 rounded"
                        />
                      </div>
                ...
                <button
                  type="button"
                  className="px-4 py-2 bg-blue-500 text-white mt-3 rounded-lg mb-5"
                  onClick={addVariant}
                >
                  {editIndex !== null ? "Update Variant" : "Add Variant"}
                </button>
                {productData.variants.length > 0 && (
                  <div
                    className="bb-table border-none border-[1px] md:border-solid border-[#eee] rounded-none md:rounded-[20px] overflow-hidden max-[1399px]:overflow-y-auto aos-init aos-animate"
                    data-aos="fade-up"
                      ...
                      <tbody>
                        {productData.variants.map((variant, index) => (
                          ...
                          >
                            ...
                            <td data-label="Price" className="p-[12px]">
                              ₹{variant.price}
                            </td>
            ...
            <div className="w-full px-[12px]">
              <div className="input-button">
                <button
                  type="submit"
                  className="block px-6 py-3 bg-blue-500 text-sm text-white mt-6 mx-auto w-auto rounded-lg hover:bg-transparent hover:border-[#3d4750] hover:text-secondary border"
                >
