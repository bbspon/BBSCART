...
//     if (!r?.data?.ok) throw new Error(r?.data?.message || "Save failed");
//     toast.success("Outlet details uploaded successfully!", {
//       duration: 4500, // disappears after 2.5s
//     });
...
export default function TerritoryHeadForm() {
  ...
  const saveStep1AndNext = async () => {
    try {
      ...
      setStep(2);
      toast.success("✅ PAN uploaded successfully!", { duration: 4500 });
    } catch (e) {
      console.error(e);
      alert(e?.response?.data?.message || e.message || "Save failed");
    }
  ...
  const maybeGenerateBpcId = async () => {
    ...
    let run = "";
    try {
      const url = `${import.meta.env.VITE_API_URL}/api/territory-heads/next-seq?state=${encodeURIComponent(
        state2
    ...
    setBpcId(code);
    localStorage.setItem("bpcId", code);
  };
  ...
  const saveStep2AndNext = async () => {
    try {
      ...
      toast.success("✅ Aadhaar & BPC ID saved!", { duration: 4500 });
      setStep(3);
    } catch (e) {
      console.error(e);
      alert("Save failed");
  ...
  const saveGstAndNext = async () => {
    try {
      ...
      if (!r?.data?.ok) throw new Error(r?.data?.message || "Save failed");
      setStep(4);
      toast.success("✅ GST uploaded successfully!", { duration: 4500 });
    } catch (e) {
      console.error(e);
      alert("Save failed");
    } finally {
  ...
  const saveBankDetails = async () => {
    ...
    try {
      ...
      if (!response?.data?.ok)
        throw new Error(response?.data?.message || "Save failed");
      toast.success("Bank details uploaded successfully!", { duration: 4500 });
      setStep(5);
    } catch (error) {
      console.error("Error saving bank details:", error);
      alert("Failed to save bank details.");
  ...
  const saveOutletAndFinish = async () => {
    ...
    if (!r?.data?.ok) throw new Error(r?.data?.message || "Save failed");
    toast.success("Outlet details uploaded successfully!", { duration: 4500 });
  ...
  return (
    <div>
          ...
          <Row>
            <Col md={6} className="mb-3">
              ...
              <Form.Control
                ...
                className="border border-dark rounded-lg my-3"
                style={{ border: "0.1px solid #333", boxShadow: "none" }}
              />
            </Col>
            <Col md={6} className="mb-3">
              <Form.Label>Surname (Last Name)</Form.Label>
      ...
      {step === 2 && (
        ...
        >
          <h5 className="mb-3 text-center">Step 2: Aadhaar & Address</h5>
