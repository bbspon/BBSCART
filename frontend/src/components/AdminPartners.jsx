...
import { FiSearch, FiChevronDown, FiPlus, FiBell, FiEdit, FiTrash2 } from "react-icons/fi";
...
const [editData, setEditData] = useState({
  ...
  const cities = useMemo(() => ["All", ...Array.from(new Set(partners.map((p) => p.city)))], [partners]);
  const types = useMemo(() => ["All", ...Array.from(new Set(partners.map((p) => p.type)))], [partners]);
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      data = data.filter((p) =>
        [p.name, p.businessName, p.id, p.email, p.contact, p.city].some((field) => field.toLowerCase().includes(q))
      );
    }
    ...
    // sorts
    if (sortBy === "joinedDesc") data.sort((a, b) => new Date(b.joinedAt) - new Date(a.joinedAt));
    if (sortBy === "joinedAsc") data.sort((a, b) => new Date(a.joinedAt) - new Date(b.joinedAt));
  ...
  const confirmAction = async (action, partner) => {
    ...
    if (value) {
      ...
      Swal.fire({ title: `${action} in progress...`, timer: 800, showConfirmButton: false });
      setTimeout(() => {
        setPartners((prev) => prev.map((p) => (p.id === partner.id ? { ...p, status: action === 'Suspend' ? 'Suspended' : 'Active' } : p)));
  ...
  const exportCSV = () => {
    const rows = [
      ["id", "name", "businessName", "type", "category", "contact", "email", "city", "status", "joinedAt", "revenue"],
      ...filtered.map((p) => [p.id, p.name, p.businessName, p.type, p.category, p.contact, p.email, p.city, p.status, p.joinedAt, p.revenue]),
    ];
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-gray-800">Business Partners</h1>
            <p className="text-sm text-gray-500">Manage BBSCART sellers, vendors, franchises & affiliates</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <StatCard title="Pending Approvals" value={partners.filter((p) => p.status === 'Pending').length} />
        <StatCard title="Total Revenue (est)" value={partners.reduce((s, p) => s + p.revenue, 0)} currency />
      </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-3">
          <div className="flex-1 flex items-center gap-2">
            ...
            <div className="flex gap-2">
              ...
              <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="border rounded-md px-3 py-2">
                {types.map((t) => (
                  <option key={t} value={t}>{t}</option>
                ))}
              </select>
              <select value={cityFilter} onChange={(e) => setCityFilter(e.target.value)} className="border rounded-md px-3 py-2">
                {cities.map((c) => (
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              <button className="px-3 py-2 border rounded-md" onClick={() => { setViewMode(viewMode === 'table' ? 'cards' : 'table'); }}>
                Toggle View
              </button>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        {loading ? (
          <div className="py-12 text-center text-gray-500">Loading partners...</div>
        ) : (
          <>
            {/* Desktop table */}
                ...
                <tbody>
                  {visible.map((p) => (
                    <tr key={p.id} className="border-t">
                      <td className="py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-md bg-indigo-50 flex items-center justify-center font-semibold text-indigo-600">{p.name.split(' ').map(s=>s[0]).join('').slice(0,2)}</div>
                          <div>
                            <div className="font-medium text-gray-800">{p.businessName}</div>
                            <div className="text-xs text-gray-500">{p.name} • {p.id}</div>
                          </div>
                        </div>
                      </td>
                      <td>{p.type}</td>
                      ...
                      <td className="text-right">
                        <div className="inline-flex items-center gap-2">
                          <button className="px-2 py-1 border rounded-md text-sm" onClick={() => openPartner(p)}>View</button>
                          <button className="px-2 py-1 border rounded-md text-sm" onClick={() => confirmAction('Suspend', p)}>Suspend</button>
                          <button className="px-2 py-1 border rounded-md text-sm text-red-600" onClick={() => deletePartner(p)}>Delete</button>
                        </div>
                      </td>
                    </tr>
                  ))}
              ...
              {visible.map((p) => (
                <div key={p.id} className="p-3 border rounded-lg">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-md bg-indigo-50 flex items-center justify-center font-semibold text-indigo-600">{p.name.split(' ').map(s=>s[0]).join('').slice(0,2)}</div>
                      <div>
                        <div className="font-medium">{p.businessName}</div>
                        <div className="text-xs text-gray-500">{p.name} • {p.city}</div>
                      </div>
                    </div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <button className="flex-1 px-3 py-2 border rounded-md" onClick={() => openPartner(p)}>Details</button>
                    <button className="px-3 py-2 border rounded-md" onClick={() => confirmAction('Suspend', p)}>Suspend</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-4 flex items-center justify-between">
              <div className="text-sm text-gray-500">Showing {(page-1)*PAGE_SIZE + 1} - {Math.min(page*PAGE_SIZE, filtered.length)} of {filtered.length}</div>
              <div className="flex items-center gap-2">
                <button className="px-3 py-1 border rounded-md" disabled={page===1} onClick={() => setPage((p)=>Math.max(1,p-1))}>Prev</button>
                <div className="px-3 py-1 border rounded-md">{page} / {pageCount}</div>
                <button className="px-3 py-1 border rounded-md" disabled={page===pageCount} onClick={() => setPage((p)=>Math.min(pageCount,p+1))}>Next</button>
              </div>
            </div>
          </>
        )}
  ...
  <div className="fixed inset-0 z-50 flex">
    ...
    <div className="w-full sm:w-96 bg-white p-4 border-l overflow-y-auto">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-lg font-semibold">{selectedPartner.businessName}</h2>
        ...
        <Section title="Overview">
          {isEditMode ? (
            ...
            <>
              ...
              <KeyVal
                label="Joined"
                value={new Date(selectedPartner.joinedAt).toLocaleDateString()}
              />
            </>
          )}
        </Section>
  ...
  return (
    <div className="bg-white rounded-lg p-4 shadow-sm flex items-center justify-between">
      <div>
        ...
        <div className="text-lg font-semibold mt-1">{currency ? `₹${Number(value).toLocaleString()}` : value}</div>
      </div>
      <div className="text-indigo-600 text-3xl">{title === 'Total Partners' ? <HiOutlineUsers /> : <RiBuilding4Line />}</div>
    </div>
  );
}
...
function FiDownloadIconPlaceholder(){
  ...
  return <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5 5 5-5M12 15V3" /></svg>;
}
