...
const Login = () => {
  ...
  const validateLogin = () => {
    ...
    // Email validation
    if (!loginData.lemail.trim()) {
      formErrors.lemail = "Email is required";
    } else if (
      !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(loginData.lemail)
    ) {
      formErrors.lemail = "Enter a valid email address";
    }

    // Password validation
    if (!loginData.lpassword.trim()) {
      formErrors.lpassword = "Password is required";
    } else if (loginData.lpassword.length < 8) {
      formErrors.lpassword = "Password must be at least 8 characters long";
    } else if (
      !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*])[A-Za-z\d!@#$%^&*]+$/.test(
        loginData.lpassword
      )
    ) {
      formErrors.lpassword =
