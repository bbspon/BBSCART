...
function MyAccount() {
    ...
    if (user) {
      ...
      setImagePreview(
        user?.details?.profilePic
          ? import.meta.env.VITE_API_URL + "" + user?.details?.profilePic
          : ""
      );
  ...
  useEffect(() => {
    const fetchStates = async () => {
      const selectedCountry = countries.find(
        (c) => c.label === userData.address.country
      );
      if (selectedCountry) {
  ...
  useEffect(() => {
    const fetchCities = async () => {
      ...
      const selectedState = states.find(
        (s) => s.label === userData.address.state
      );
  ...
  return (
    <>
      <div className="input-box-form w-[90%] mx-auto my-5 border-2 border-[#800c0c] py-5 px-8 rounded-lg shadow-lg">
        <h4 className="font-quicksand tracking-[0.03rem] leading-[1.2] text-[20px] font-bold text-secondary mt-8 mb-4">
          Profile Details
        </h4>
          ...
          <div className="flex flex-wrap mx-[-12px]">
            ...
            <div className="min-[992px]:w-[50%] w-full px-[12px]">
              <div className="input-item mb-[24px]">
                ...
                <Select
                  ...
                  value={
                    countries.find(
                      (option) => option.label === userData?.address?.country
                    ) || null
                  }
                  onChange={(option, action) =>
                    handleSelectChange(option, { name: "country" })
                  }
            ...
            <div className="min-[992px]:w-[50%] w-full px-[12px]">
              <div className="input-item mb-[24px]">
                ...
                <Select
                  ...
                  value={
                    states.find(
                      (option) => option.label === userData?.address?.state
                    ) || null
                  }
                  onChange={(option, action) =>
                    handleSelectChange(option, { name: "state" })
                  }
            ...
            <div className="min-[992px]:w-[50%] w-full px-[12px]">
              <div className="input-item mb-[24px]">
                ...
                <Select
                  ...
                  value={
                    cities.find(
                      (option) => option.label === userData?.address?.city
                    ) || null
                  }
                  onChange={(option, action) =>
            ...
            <div className="w-full px-[12px]">
              <div className="input-button">
                <button
                  type="submit"
                  className="bb-btn-2 inline-block py-[10px] px-[25px] text-[14px] font-medium text-white bg-[#800c0c] rounded-[10px] hover:bg-transparent hover:border-[#3d4750] hover:text-secondary border"
                >
                  Save
                </button>
              </div>
