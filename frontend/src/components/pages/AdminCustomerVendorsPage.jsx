...
export default function AdminCustomerVendorsPage() {
  ...
  const fetchList = async () => {
    setLoading(true);
    try {
      const url = listEndpointForStatus(status);
      const r = await axios.get(url);
      const data = r?.data?.data || [];
      setRows(Array.isArray(data) ? data : []);
    } catch (e) {
      console.error("CBV list error:", e?.response?.data || e.message);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const openReview = async (id) => {
    ...
    try {
      ...
      console.error("CBV get detail error:", e?.response?.data || e.message);
      setSelectedDoc(null);
    } finally {
            ...
            {loading ? (
              ...
              pageData.map((r) => (
                <tr key={r._id} style={{ borderTop: "1px solid #eee" }}>
                  ...
                  <td style={{ padding: 10 }}>
                    {fmtTime(r.updated_at, r.submitted_at)}
                  </td>
                  <td style={{ padding: 10 }}>
                    <button onClick={() => openReview(r._id)}>View</button>
                  </td>
                </tr>
              ))
            )}
      ...
      {reviewOpen && (
        ...
        >
          ...
          >
            ...
            >
              <h3 style={{ margin: 0 }}>Customer Vendor Details</h3>
              ...
              <>
                <div style={{ marginTop: 8 }}>
                  ...
                  <div>
                    ...
                    <b> Active:</b> {String(selectedDoc.is_active)}
                  </div>
                </div>
