...
export default function AdminCustomerVendorRequestsPage() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      console.error("CBV requests list error:", e?.response?.data || e.message);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      const pan = safe(r.pan_number).toLowerCase();
      const gst = safe(r.gst_number).toLowerCase();
  ...
  const doReject = async () => {
    ...
    if (!rejectReason.trim()) {
      alert("Enter a reason to reject");
      return;
    }
    setActionBusy(true);
    try {
  ...
  const fmtTime = (v1, v2) => {
    ...
    // prefers submitted_at; falls back to updated_at if needed
  };

  return (
    <div style={{ padding: 16 }}>
      <h2>Customer Become Vendor – Requests</h2>
      ...
      >
        <input
          placeholder="Search name / PAN / GST / City / State"
          value={q}
          onChange={(e) => {
            setPage(1);
            setQ(e.target.value);
            ...
            {loading ? (
              ...
              pageData.map((r) => (
                <tr key={r._id} style={{ borderTop: "1px solid #eee" }}>
                  ...
                  <td style={{ padding: 10 }}>{safe(r.pan_number)}</td>
                  <td style={{ padding: 10 }}>{safe(r.gst_number)}</td>
                  <td style={{ padding: 10 }}>
                  ...
                  <td style={{ padding: 10 }}>{safe(r.application_status)}</td>
                  <td style={{ padding: 10 }}>
                    {fmtTime(r.submitted_at, r.updated_at)}
                  </td>
                  <td style={{ padding: 10 }}>
                    <button onClick={() => openReview(r._id)}>Review</button>
                  </td>
                </tr>
              ))
            )}
      ...
      {reviewOpen && (
        ...
        >
          ...
          >
            ...
            >
              <h3 style={{ margin: 0 }}>Review Customer Vendor</h3>
              <button onClick={closeReview} disabled={actionBusy}>
                ✕
              </button>
            </div>
            ...
            {reviewLoading ? (
              ...
              <>
                <div style={{ marginTop: 8 }}>
                  ...
                  <div>
                    <b>PAN:</b> {safe(selectedDoc.pan_number)}
                  </div>
                  <div>
                    <b>Aadhaar:</b> {safe(selectedDoc.aadhar_number)}
                  </div>
                  <div>
                    <b>GSTIN:</b> {safe(selectedDoc.gst_number)}
                  </div>
                  <div>
                    <b>Status:</b> {safe(selectedDoc.application_status)}
                  </div>
                  <div>
                    <b>Submitted:</b>{" "}
                    {fmtTime(selectedDoc.submitted_at, selectedDoc.updated_at)}
                  </div>
                </div>
                  ...
                  <div>
                    <label>
                      ...
                      <textarea
                        ...
                        placeholder="Any internal note for approval"
                      />
                    </label>
                    <div style={{ marginTop: 8 }}>
                      <button
